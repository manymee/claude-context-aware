...
## Project
- Stop hook plugin for Claude Code — monitors context window usage, recommends handoff
- `default-config.json` — source of truth for default thresholds/settings
- `hooks/context-monitor.sh` — stop hook script (requires jq)
- `hooks/context-monitor.test.sh` — test suite, run with `bash hooks/context-monitor.test.sh`
